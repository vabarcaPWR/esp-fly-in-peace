...
static uint8_t s_sd_gpio = 0;
static int16_t s_silence_buf[MAX98357_SILENCE_SAMPLES];
...
esp_err_t max98357_hw_write(const int16_t *buf, size_t samples)
{
    ...
    return i2s_channel_write(s_tx_chan, buf, samples * sizeof(int16_t), &bytes_written,
...
void max98357_hw_mute(void)
{
    ...
    size_t bytes_written = 0;
    i2s_channel_write(s_tx_chan, s_silence_buf, sizeof(s_silence_buf), &bytes_written,
                      pdMS_TO_TICKS(MAX98357_WRITE_TIMEOUT_MS));
}
...
void max98357_hw_shutdown(bool enable)
{
    ...
    gpio_set_level(s_sd_gpio, enable ? 1 : 0);
}
