...
{
    if (!mpu6050_ctx.i2c_dev_handle)
        return ESP_ERR_INVALID_STATE;

    uint8_t buf[2] = {reg, value};
    return i2c_master_transmit(mpu6050_ctx.i2c_dev_handle, buf, sizeof(buf), MPU6050_I2C_TIMEOUT_MS);
}

static esp_err_t mpu6050_read_register(uint8_t reg, uint8_t *data, size_t len)
{
    if (!mpu6050_ctx.i2c_dev_handle || !data)
        return ESP_ERR_INVALID_STATE;

    return i2c_master_transmit_receive(mpu6050_ctx.i2c_dev_handle, &reg, 1, data, len, MPU6050_I2C_TIMEOUT_MS);
}
