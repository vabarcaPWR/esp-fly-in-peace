...
{
    app_context.led = get_led("single");
    if (!app_context.led || !app_context.led->init || !app_context.led->set_state)
        return ESP_ERR_NOT_FOUND;

    return app_context.led->init();
}
...
static void ble_connection_led_state_handler(bool connected, uint16_t conn_handle)
{
    ...
    if (!app_context.led || !app_context.led->set_state)
        return;

    led_state_e state = connected ? LED_STATE_BLE_CONNECTED : LED_STATE_BLE_DISCONNECTED;
    app_context.led->set_state(state);
}

static esp_err_t initialize_sensors(void)
{
#ifdef CONFIG_SENSOR_MS5611
    app_context.baro_sensor = get_baro_sensor("ms5611");
#endif
#ifdef CONFIG_SENSOR_BMP390
    app_context.baro_sensor = get_baro_sensor("bmp390");
#endif
    if (app_context.baro_sensor)
    {
        esp_err_t result = app_context.baro_sensor->init();
        if (result != ESP_OK)
        {
            ESP_LOGW(TAG, "Baro sensor init failed: 0x%x", result);
            app_context.baro_sensor = NULL;
        }
    }
...
static esp_err_t configure_modules_usage(void)
{
    ...
    if (app_context.led && app_context.led->set_state)
        app_context.led->set_state(ble_nus_is_connected() ? LED_STATE_BLE_CONNECTED : LED_STATE_BLE_DISCONNECTED);
...
{
    if (!threads || !app_context.baro_sensor)
        return ESP_OK;

    BaseType_t ok = xTaskCreate(baro_task_fn, "baro", BARO_TASK_STACK_SIZE, (void *)app_context.baro_sensor,
...
void app_main(void)
{
    ...
    if (ret)
        ESP_LOGI(TAG, "pipeline running: baro=%s", app_context.baro_sensor ? "yes" : "no");
}
