...
TEST_SOURCE_FILE("../components/sound/src/piezo/src/piezo_model.c")

#define FLOAT_TOL 0.01f

static piezo_tone_curve_t default_curve(void)
{
    const piezo_tone_config_t *cfg = piezo_config_get_defaults();
    return cfg->curve;
}

static piezo_tone_thresholds_t default_thresholds(void)
{
    const piezo_tone_config_t *cfg = piezo_config_get_defaults();
...
{
    piezo_tone_curve_t curve = default_curve();
    piezo_tone_thresholds_t thresh = default_thresholds();
    piezo_tone_output_t out = {0};

    piezo_model_compute(&curve, &thresh, true, 1.0f, &out);
    TEST_ASSERT_EQUAL_UINT16(700, out.freq_hz);
    TEST_ASSERT_EQUAL_UINT16(420, out.cycle_ms);
    TEST_ASSERT_EQUAL_UINT8(40, out.duty_pct);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_CLIMB, out.zone);
}
...
void test_compute_between_breakpoints_interpolates_linearly(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 1.5f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_CLIMB, out.zone);
...
void test_compute_below_first_breakpoint_clamps_to_first(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, -15.0f, &out);
    TEST_ASSERT_EQUAL_UINT16(200, out.freq_hz);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_SINK, out.zone);
}
...
void test_compute_above_last_breakpoint_clamps_to_last(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 20.0f, &out);
    TEST_ASSERT_EQUAL_UINT16(1600, out.freq_hz);
    TEST_ASSERT_EQUAL_UINT16(180, out.cycle_ms);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_CLIMB, out.zone);
}
...
void test_vario_in_dead_zone_returns_silence(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 0.0f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_SILENCE, out.zone);
...
void test_vario_in_pre_lift_zone_returns_pre_lift(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 0.10f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_PRE_LIFT, out.zone);
...
void test_pre_lift_disabled_returns_silence_in_pre_lift_zone(void)
{
    ...
    piezo_model_compute(&curve, &thresh, false, 0.10f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_SILENCE, out.zone);
}
...
void test_vario_crosses_climb_on_returns_climb(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 0.15f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_CLIMB, out.zone);
...
void test_vario_just_below_climb_on_is_pre_lift(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 0.14f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_PRE_LIFT, out.zone);
}
...
void test_vario_at_climb_off_boundary_is_pre_lift(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 0.05f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_PRE_LIFT, out.zone);
}
...
void test_vario_just_below_climb_off_is_silence(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 0.04f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_SILENCE, out.zone);
}
...
void test_sink_threshold_triggers_sink_zone(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, -2.0f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_SINK, out.zone);
    TEST_ASSERT_TRUE(out.freq_hz > 0);
}
...
void test_vario_just_above_sink_on_is_silence(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, -1.9f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_SILENCE, out.zone);
}
...
{
    const piezo_tone_config_t *cfg = piezo_config_get_defaults();
    TEST_ASSERT_EQUAL(ESP_OK, piezo_config_validate(cfg));
}

void test_validate_null_config_returns_invalid_arg(void)
{
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, piezo_config_validate(NULL));
}

void test_validate_unsorted_breakpoints_rejected(void)
{
    piezo_tone_config_t cfg = *piezo_config_get_defaults();
    cfg.curve.points[1].vario_ms = cfg.curve.points[0].vario_ms;
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, piezo_config_validate(&cfg));
}
...
{
    piezo_tone_config_t cfg = *piezo_config_get_defaults();
    cfg.curve.count = PIEZO_MAX_TONE_POINTS + 1;
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, piezo_config_validate(&cfg));
}
...
{
    piezo_tone_thresholds_t thresh = default_thresholds();
    piezo_tone_output_t out = {.freq_hz = 999};

    piezo_model_compute(NULL, &thresh, true, 1.0f, &out);
...
void test_compute_null_output_does_not_crash(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, 1.0f, NULL);
}
...
void test_sink_zone_continuous_tone_has_zero_cycle(void)
{
    ...
    piezo_model_compute(&curve, &thresh, true, -5.0f, &out);
    TEST_ASSERT_EQUAL(PIEZO_ZONE_SINK, out.zone);
    TEST_ASSERT_EQUAL_UINT16(0, out.cycle_ms);
    TEST_ASSERT_EQUAL_UINT8(100, out.duty_pct);
}
